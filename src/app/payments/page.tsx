...
import { getPaymentsByCustomerId } from "@/libs/api";
...
export default function PaymentsPage() {
    ...
    useEffect(() => {
        async function fetchOrders() {
            if (user?.id) {
                try {
                    const response = await getPaymentsByCustomerId(user?.id);
                            ...
                            {orders.map((order: any) => (
                                <div key={order.id} className="p-4">
                                    <div className="md:grid md:grid-cols-5 md:gap-4 flex flex-col space-y-2 md:space-y-0 md:items-center">
                                        ...
                                        <div className="font-medium">${Number(order.order_details.total_price).toFixed(2)}</div>
                                    ...
                                    {expandedOrder === order.id && (
                                        <div className="mt-4 pt-4 border-t border-gray-100">
                                            ...
                                            <div className="space-y-3">
                                                {order.order_details.items.map((item: any) => (
